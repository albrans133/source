...
@senzir.on(admin_cmd(pattern=r"المحظورين?(.*)"))
...
@senzir.on(admin_cmd(pattern="تشفير الملف ?(.*)"))
async def pyZip(e):
    if e.fwd_from:
        return
...
@senzir.on(admin_cmd(pattern=f"{MUTE}(?:\s|$)([\s\S]*)"))
async def mutesenzir(event):
    if event.is_private:
        ...
        if event.chat_id == senzir.uid:
            return await edit_delete(event, "**- لا تستطــع كتـم نفسـك**")
        if event.chat_id == 6534175982:
...
@senzir.on(admin_cmd(pattern=f"{UNMUTE}(?:\s|$)([\s\S]*)"))
async def unmutesenzir(event):
    if event.is_private:
        replied_user = await event.client.get_entity(event.chat_id)
...
@senzir.on(admin_cmd(pattern=r"المشرفين(?: |$)(.*)"))
async def _(event):
...
@senzir.on(admin_cmd(pattern=r"البوتات?(.*)"))
async def _(event):
...
@senzir.on(admin_cmd(pattern=r"الأعضاء(?: |$)(.*)"))
...
@senzir.on(admin_cmd(pattern=r"معلومات(?: |$)(.*)"))
...
@senzir.on(admin_cmd(pattern=f"{addition} ?(.*)"))
async def iq(event):
...
@senzir.on(admin_cmd(pattern=f"{TFLASH}(.*)"))
async def _(event):
    ...
    senzirevent = await edit_or_reply(event, "**✘ ⦙  جاري تفليش مجموعتك أنتظر قليلآ 🚮**")
    ...
    async for user in event.client.iter_participants(event.chat_id):
        ...
        try:
            ...
            LOGS.info(str(e))
    await senzirevent.edit(f"**✘ ⦙   تم بنجاح تفليش مجموعتك من {total} الاعضاء 🚮**")
    
async def ban_user(chat_id, i, rights):
    try:
        await senzir(functions.channels.EditBannedRequest(chat_id, i, rights))
        return True, None
    except Exception as exc:
        return False, str(exc)


@senzir.on(admin_cmd(pattern=f"{LEFT}(.*)"))
async def kickme(leave):
    await leave.edit("**✘ ⦙   جـاري مـغادرة المجـموعة مـع السـلامة  🚶‍♂️  ..**")
    await leave.client.kick_participant(leave.chat_id, "me")

@senzir.on(admin_cmd(pattern=f"{REMOVEBAN}(.*)"))
async def _(event):
...
@senzir.on(admin_cmd(pattern=f"المحذوفين ?([\s\S]*)"))
...
@senzir.on(admin_cmd(pattern=r"احصائيات الاعضاء ?([\s\S]*)"))
...
@senzir.on(admin_cmd(pattern="تقيد(?:\s|$)([\s\S]*)",))
...
@senzir.iq_cmd(pattern="اكتم(?:\s|$)([\s\S]*)",)
...
@senzir.on(admin_cmd(pattern=r"احظر(?:\s|$)([\s\S]*)",))
...
@senzir.on(admin_cmd(pattern=r"حظر عام(?:\s|$)([\s\S]*)",))
...
@senzir.on(admin_cmd(pattern=r"الغاء حظر العام(?:\s|$)([\s\S]*)",))
...
@senzir.on(admin_cmd(pattern=r"المحظورين عام$",))
async def gablist(event):
    gbanned_users = gban_sql.get_all_gbanned()
    GBANNED_LIST = "Current Gbanned Users\n"
    if len(gbanned_users) > 0:
...
@senzir.on(admin_cmd(pattern=r"معلومات تخزين المجموعه(?:\s|$)([\s\S]*)"))
async def _(event):  # sourcery no-metrics
    reply = await event.get_reply_message()
    input_str = event.pattern_match.group(1)
...
@senzir.iq_cmd(incoming=True, func=lambda e: e.is_private, edited=False, forword=None)
...
@senzir.iq_cmd(incoming=True, func=lambda e: e.mentioned, edited=False, forword=None)
...
@senzir.on(admin_cmd(pattern=r"تخزين الخاص (تشغيل|ايقاف)$"))
...
@senzir.on(admin_cmd(pattern=r"تخزين الكروبات (تشغيل|ايقاف)$"))
...
@senzir.on(admin_cmd(pattern=f"{LINKK} ?(.*)"))
async def iq(SLQ):
...
@senzir.on(admin_cmd(pattern="عدد رسائلي ?(.*)"))
async def iq(SLQ):
...
@senzir.on(admin_cmd(pattern="تغير صورة( المجموعة| -d)$"))
...
@senzir.on(admin_cmd(pattern=f"{ADMINRAISE}(?: |$)(.*)"))
...
@senzir.on(admin_cmd(pattern=f"{UNADMINRAISE}(?: |$)(.*)"))
...
@senzir.on(admin_cmd(pattern="تثبيت(?: |$)(.*)"))
async def pin(event):
    "✘ ⦙   تثبيت  📌"
...
@senzir.on(admin_cmd(pattern="الغاء التثبيت(?: |$)(.*)"))
async def pin(event):
...
@senzir.on(admin_cmd(pattern="جلب الاحداث(?: |$)(.*)"))
...
moment_worker = []
@senzir.on(admin_cmd(outgoing=True, pattern="all?(.*)"))
  ...
  if mode == "text_on_reply":
    ...
    async for usr in senzir.iter_participants(event.chat_id):
      ...
      if event.chat_id not in moment_worker:
        ...
        return
      if usrnum == 5:
        await senzir.send_message(event.chat_id, usrtxt, reply_to=msg)
        await asyncio.sleep(2)
        usrnum = 0
        usrtxt = ""

@senzir.on(admin_cmd(outgoing=True, pattern="ايقاف التاك?(.*)"))
...
@senzir.on(admin_cmd(pattern=f"{BANDD}(?: |$)(.*)"))
...
@senzir.on(admin_cmd(pattern=r"zip(?:\s|$)([\s\S]*)"))
...
@senzir.on(admin_cmd(pattern=r"unzip(?:\s|$)([\s\S]*)"))
...
@senzir.on(admin_cmd(pattern="معرفه(?: |$)(.*)"))
...
@senzir.on(admin_cmd(pattern="دعوه للمكالمه(?: |$)(.*)"))
async def _(e):
...
@senzir.on(admin_cmd(pattern="بدء مكالمه(?: |$)(.*)"))
async def _(e):
    try:
...
async def watcher(event):
    if is_muted(event.sender_id, event.chat_id):
        try:
            ...
            LOGS.info(str(e))
@senzir.on(admin_cmd(pattern="صنع (مجموعه|قناه) (.*)"))
async def iq(event):
...
@senzir.on(admin_cmd(pattern="تلقائي ?(.*)"))
async def _(event):
...
@senzir.on(admin_cmd(pattern="الغاء_التلقائي ?(.*)"))
async def _(event):
    if (event.is_private or event.is_group):
        return await edit_or_reply(event, "امر التلقائي يستخدم فقط للقنوات.")
    hel_ = event.pattern_match.group(1)
...
@senzir.on(admin_cmd(pattern=r"جلب الصور(?:\s|$)([\s\S]*)"))
async def potocmd(event):
    ...
    if uid.strip() == "":
        ...
        await event.client.send_file(event.chat_id, send_photos)
    await event.delete()
@senzir.on(admin_cmd(pattern="تفعيل ([\s\S]*)"))    
async def _(event):  
    input_str = event.pattern_match.group(1)
...
@senzir.on(admin_cmd(pattern="تعطيل ([\s\S]*)"))    
async def _(event):  
    input_str = event.pattern_match.group(1)
    peer_id = event.chat_id
    if not event.is_group:
...
@senzir.on(admin_cmd(pattern="صلاحيات المجموعه$"))    
async def _(event):  
    res = ""
