...
async def all_groups_id(cat):
    ...
    return catgroups
@senzir.on(admin_cmd(pattern="ازاله التوجيه(?: |$)(.*)"))    
async def _(event):
    try:
        await event.delete()
...
@senzir.on(admin_cmd(pattern=f"{ALLGROUB} ?(.*)$"))    
...
async def getTranslate(text, **kwargs):
    ...
    for _ in range(10):
        try:
            ...
            await sleep(0.1)
    return result
@senzir.iq_cmd(incoming=True, groups_only=True)
async def _(event):
    if not CHAT_FLOOD:
        return
...
@senzir.on(admin_cmd(pattern=f"{ALLPRIVATE} ?(.*)$"))    
...
@senzir.on(admin_cmd(pattern="كتابه وهمي(?:\s|$)([\s\S]*)"))
...
@senzir.on(admin_cmd(pattern="تكرار (.*)"))    
...
@senzir.on(admin_cmd(pattern="مؤقته (\d*) ([\s\S]*)"))    
async def selfdestruct(destroy):
    ...
    await sleep(ttl)
    await smsg.delete()
def senzirveFile(input_file_name):
...
@senzir.on(admin_cmd(pattern="ترحيب(?:\s|$)([\s\S]*)"))    
...
@senzir.on(admin_cmd(pattern="مسح الترحيبات(?: |$)(.*)"))    
...
@senzir.on(admin_cmd(pattern="ترحيباتي(?: |$)(.*)"))    
...
@senzir.on(admin_cmd(pattern="موقع(?: |$)(.*)"))    
async def gps(event):
...
async def ocr_senzir(filename, overlay=False,api_key="K88942796188957", language="eng"):
    payload = {"isOverlayRequired": overlay,"apikey": api_key,"language": language}
    with open(filename, "rb") as f:
        r = requests.post("https://api.ocr.space/parse/image",files={filename: f},data=payload)
    return r.json()
@senzir.on(admin_cmd(pattern="فيديو وهمي(?:\s|$)([\s\S]*)"))
async def _iq(senzir):
    iq = senzir.pattern_match.group(1)
    if not (iq or iq.isdigit()):
        iq = 100
        ...
        try:
            ...
            try:
                ...
                return await event.edit("**اكتب الامر بشكل صحيح**")
    await senzir.edit(f"**تم بدء وضع فيديو وهمي لـ {iq} من الثوانـي**")
    async with senzir.client.action(senzir.chat_id, "record-video"):
        await asyncio.sleep(iq)
@senzir.on(admin_cmd(pattern="ازعاج(?: |$)(.*)"))    
...
@senzir.on(admin_cmd(pattern="الغاء الازعاج( -a)?"))    
async def echo(event):
    input_str = event.pattern_match.group(1)
    if input_str:
        lecho = get_all_echos()
        ...
        try:
            ...
            await edit_or_reply(event, "**⁂ ⦙   تـم ايقاف وضـع الازعاج على الجميع بنجاح ✅**")
@senzir.on(admin_cmd(pattern="المزعجهم( -a)?$"))    
async def echo(event):  
    input_str = event.pattern_match.group(1)
    private_chats = ""
    ...
    if not (iq or iq.isdigit()):
        ...
        try:
            ...
            try:
                ...
                return await event.edit("**اكتب الامر بشكل صحيح**")
    await senzir.edit(f"**تم بدء وضع صوت وهمي لـ {iq} من الثوانـي**")
    async with senzir.client.action(senzir.chat_id, "record-audio"):
        await asyncio.sleep(iq)
@senzir.on(admin_cmd(pattern="تنظيف(?:\s|$)([\s\S]*)"))
...
@senzir.on(admin_cmd(pattern="لعب وهمي(?:\s|$)([\s\S]*)"))
async def _iq(senzir):
    iq = senzir.pattern_match.group(1)
    if not (iq or iq.isdigit()):
        iq = 100
        ...
        try:
            ...
            try:
                ...
                return await event.edit("**اكتب الامر بشكل صحيح**")
    await senzir.edit(f"**تم بدء وضع لعب وهمي لـ {iq} من الثوانـي**")
    async with senzir.client.action(senzir.chat_id, "game"):
        await asyncio.sleep(iq)  
@senzir.iq_cmd(pattern="قرائه(?:\s|$)([\s\S]*)",)
async def ocriq(senzirevent):
    iqqevent = await edit_or_reply(senzirevent, "جاري قرائه الصوره ...")
    if not os.path.isdir(Config.TEMP_DIR):
        os.makedirs(Config.TEMP_DIR)
    lang_code = senzirevent.pattern_match.group(1)
    downloaded_file_name = await senzirevent.client.download_media(await senzirevent.get_reply_message(), Config.TEMP_DIR)
    test_file = await ocr_senzir(filename=downloaded_file_name, language=lang_code)
    try:
...
@senzir.on(admin_cmd(pattern="اضف رد ([\s\S]*)")) 
...
@senzir.on(admin_cmd(pattern="موقع وهمي(?:\s|$)([\s\S]*)"))
async def _iq(senzir):
    iq = senzir.pattern_match.group(1)
    if not (iq or iq.isdigit()):
        iq = 100
        ...
        try:
            ...
            try:
                ...
                return await event.edit("**اكتب الامر بشكل صحيح**")
    await senzir.edit(f"**تم بدء وضع موقع وهمي لـ {iq} من الثوانـي**")
    async with senzir.client.action(senzir.chat_id, "location"):
        await asyncio.sleep(iq)
@senzir.on(admin_cmd(pattern="جميع الردود(?: |$)(.*)"))    
...
@senzir.on(admin_cmd(pattern="مسح رد ([\s\S]*)")) 
...
@senzir.on(admin_cmd(pattern="مسح جميع الردود(?: |$)(.*)"))    
...
@senzir.on(admin_cmd(pattern="تكلم(?:\s|$)([\s\S]*)"))    
async def _(event):
    input_str = event.pattern_match.group(1)
    start = datetime.now()
    reply_to_id = await reply_id(event)
...
@senzir.on(admin_cmd(pattern="تحذير تكرار(?:\s|$)([\s\S]*)"))
async def _(event):
    input_str = event.pattern_match.group(1)
...
@senzir.on(admin_cmd(pattern="ترجمه ([\s\S]*)"))
async def _(event):
    input_str = event.pattern_match.group(1)
    if event.reply_to_msg_id:
        previous_message = await event.get_reply_message()
...
@senzir.on(admin_cmd(pattern="تاريخ الرساله(?: |$)(.*)"))    
async def _(event):
...
@senzir.on(admin_cmd(pattern="جهه اتصال وهمي(?:\s|$)([\s\S]*)"))
async def _iq(senzir):
    iq = senzir.pattern_match.group(1)
    if not (iq or iq.isdigit()):
        iq = 100
    ...
    async with senzir.client.action(senzir.chat_id, "contact"):
        await asyncio.sleep(iq)  
@senzir.on(admin_cmd(pattern="ماركدون(?:\s|$)([\s\S]*)"))    
async def _(event):
    reply_to_id = await reply_id(event)
    ...
    await event.delete()
@senzir.on(admin_cmd(pattern="حذف(?:\s|$)([\s\S]*)"))
async def iq(cloneiq):  
    chat = await cloneiq.get_input_chat()
    msgs = []
    count = 0
    ...
    await sleep(5)
    await hi.delete()
@senzir.on(admin_cmd(pattern="معرفات 100(?: |$)(.*)"))
async def iq(senzir):
    mentions = senzir.text[8:]
    chat = await senzir.get_input_chat()
    async for x in senzir.client.iter_participants(chat, 100):
        mentions += f" - @{x.username} ⦙ "
    await senzir.client.send_message(senzir.chat_id, mentions)
    await senzir.delete()
@senzir.on(admin_cmd(pattern="معرفات 200(?: |$)(.*)"))
async def iq(senzir):
    mentions = senzir.text[8:]
    chat = await senzir.get_input_chat()
    async for x in senzir.client.iter_participants(chat, 200):
        mentions += f" - @{x.username} ⦙ "
    await senzir.client.send_message(senzir.chat_id, mentions)
    await senzir.delete()
@senzir.on(admin_cmd(pattern="تاك 200(?: |$)(.*)"))
async def iq(senzir):
    mentions = senzir.text[8:]
    chat = await senzir.get_input_chat()
    async for x in senzir.client.iter_participants(chat, 200):
    ...
    await senzir.client.send_message(senzir.chat_id, mentions)
    await senzir.delete()
@senzir.on(admin_cmd(pattern="تاك 150(?: |$)(.*)"))
async def iq(senzir):
    mentions = senzir.text[8:]
    chat = await senzir.get_input_chat()
    async for x in senzir.client.iter_participants(chat, 150):
        mentions += f" \n⁂ ⦙ ⵧ〈[{x.first_name}](tg://user?id={x.id})〉 \n"
    await senzir.client.send_message(senzir.chat_id, mentions)
    await senzir.delete()
@senzir.on(admin_cmd(pattern="تاك 100(?: |$)(.*)"))
async def iq(senzir):
    mentions = senzir.text[8:]
    chat = await senzir.get_input_chat()
    async for x in senzir.client.iter_participants(chat, 100):
    ...
    await senzir.client.send_message(senzir.chat_id, mentions)
    await senzir.delete()
@senzir.on(admin_cmd(pattern="تاك 50(?: |$)(.*)"))
async def iq(senzir):
    mentions = senzir.text[8:]
    chat = await senzir.get_input_chat()
    async for x in senzir.client.iter_participants(chat, 50):
        mentions += f" \n⁂ ⦙ ⵧ〈[{x.first_name}](tg://user?id={x.id})〉 \n"
    await senzir.client.send_message(senzir.chat_id, mentions)
    await senzir.delete()
@senzir.on(admin_cmd(pattern="تاك 10(?: |$)(.*)"))
async def iq(senzir):
    mentions = senzir.text[8:]
    chat = await senzir.get_input_chat()
    async for x in senzir.client.iter_participants(chat, 10):
        mentions += f" \n ⁂ ⦙ ⵧ〈[{x.first_name}](tg://user?id={x.id})〉 \n"
    await senzir.client.send_message(senzir.chat_id, mentions)
    await senzir.delete()
@senzir.on(admin_cmd(pattern="احسب ([\s\S]*)"))    
...
@senzir.iq_cmd(pattern="(ازاله الخلفيه بالملصق|ازاله الخلفيه)(?:\s|$)([\s\S]*)",)
async def remove_iq(event):
    ...
    if event.reply_to_msg_id and not input_str:
        ...
        try:
            ...
            response = senzirveFile(file_name)
            os.remove(file_name)
    elif input_str:
        catevent = await edit_or_reply(event, "إزالة خلفية هذه الوسائط")
        response = senzirveURL(input_str)
    else:
    ...
    remove_bg_image = "senzir.png"
    if "image" in contentType:
        with open("senzir.png", "wb") as removed_bg_file:
            removed_bg_file.write(response.content)
    else:
        await edit_delete(catevent, f"`{response.content.decode('UTF-8')}`", 5)
        return
    if cmd == "ازاله الخلفيه بالملصق":
        file = convert_tosticker(remove_bg_image, filename="senzir.webp")
    ...
    await catevent.delete()
@senzir.on(admin_cmd(pattern="ابلاغ الادمنيه(?: |$)(.*)"))    
async def iq(event):
...
async def on_new_message(event):
    ...
    for snip in snips:
        ...
        if re.search(pattern, name, flags=re.IGNORECASE):
            ...
            break
@senzir.on(admin_cmd(pattern="خط ملصق ?(?:(.*?) ?; )?([\s\S]*)"))    
async def sticklet(event):
    ...
    try:
        ...
        pass
@senzir.on(admin_cmd(pattern="ضفدع(?:\s|$)([\s\S]*)"))    
...
@senzir.on(admin_cmd(pattern="منع(?:\s|$)([\s\S]*)"))    
async def _(event):
    text = event.pattern_match.group(1)
...
@senzir.on(admin_cmd(pattern="الغاء منع(?:\s|$)([\s\S]*)"))    
async def _(event):
    text = event.pattern_match.group(1)
...
@senzir.on(admin_cmd(pattern="قائمه المنع$"))    
async def _(event):
...
@senzir.on(admin_cmd(pattern="مسح(\s*| \d+)$"))
async def iq(msg1):
    ...
    if iq_src:
        ...
        await msg1.delete()
@senzir.on(admin_cmd(pattern="تاك بالكلام ([\s\S]*)"))    
async def iq(event):
...
@senzir.on(admin_cmd(pattern="لافته(?:\s|$)([\s\S]*)"))    
...
@senzir.on(admin_cmd(pattern="نسخ(?: |$)(.*)"))
async def iq(nshkh):
    if nshkh.fwd_from:
        return
...
@senzir.on(admin_cmd(pattern="غمق(?: |$)(.*)"))
async def iq(nshkh):
    if nshkh.fwd_from:
        return
    if nshkh.reply_to_msg_id:
        ...
        await nshkh.edit("**⁂ ⦙   قم  برد على الرساله ** ")
@senzir.on(admin_cmd(pattern="ينسخ(?: |$)(.*)"))
async def iq(nshkh):
    if nshkh.fwd_from:
        return
    if nshkh.reply_to_msg_id:
        ...
        await nshkh.edit("**⁂ ⦙   قم  برد على الرساله ** ")
@senzir.on(admin_cmd(pattern="خط سفلي(?: |$)(.*)"))
async def iq(nshkh):
    if nshkh.fwd_from:
        return
    if nshkh.reply_to_msg_id:
