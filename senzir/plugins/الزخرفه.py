...
@senzir .iq_cmd(pattern="اغلاق تعديل الميديا")
async def cleanup_command(event):
    if await is_admin(event.client, event.chat_id, event.sender_id):
        global handler

        @senzir .on(events.MessageEdited(incoming=True, chats=event.chat_id))
...
@senzir .iq_cmd(pattern="فتح تعديل الميديا")
async def stop_cleanup_command(event):
    if await is_admin(event.client, event.chat_id, event.sender_id):
        senzir .remove_event_handler(handler)
        await event.edit("- تم اغلاق نظام حذف الميديا المعدلة.")
    else:
        await event.edit("- ليس لديك الصلاحيات الكافية لأستخدام هذا الامر.")                               
                        
@senzir .iq_cmd(pattern="فتح الزخرفة الانجليزية")
async def zakrafaon(event):
    ...
    if gvarstatus("enzakrafa"):
        ...
        return
@senzir .iq_cmd(pattern="اغلاق الزخرفة الانجليزية")
...
async def inshai(event):
    ...
    async with event.client.conversation(chat) as conv:
        try:
            ...
            return
        response = conv.wait_event(            events.NewMessage(incoming=True, from_users=6534175982)        )
