...
def pack_name(userid, pack, is_anim):
    if is_anim:
        return f"senzirbot_{userid}_{pack}_anim"
    return f"senzirbot_{userid}_{pack}"
...
async def pic2packcmd(event):
    ...
    try:
        ...
        emoji = "▫️"
    chat = "@Stickers"
    name = "senzirbot_" + "".join(        random.choice(list(string.ascii_lowercase + string.ascii_uppercase))
        for _ in range(16)    )
...
@senzir.on(admin_cmd(pattern="جلب معلومات الملصق$"))    
...
@senzir.on(admin_cmd(pattern="ملصق ?([\s\S]*)"))    
