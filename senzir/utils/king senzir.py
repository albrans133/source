...
from senzir import CMD_HELP, LOAD_PLUG
...
def load_module(shortname, plugin_path=None):
    if shortname.startswith("__"):
        ...
        path = Path(f"senzir/plugins/{shortname}.py")
        checkplugins(path)
        name = "senzir.plugins.{}".format(shortname)
        ...
        if plugin_path is None:
            path = Path(f"senzir/plugins/{shortname}.py")
            name = f"senzir.plugins.{shortname}"
        else:
        ...
        spec.loader.exec_module(mod)
        # for imports
        sys.modules["senzir.plugins." + shortname] = mod
        LOGS.info(f"[تم تحميل : {shortname}] \n ———————× IQ")
...
def remove_plugin(shortname):
    ...
    try:
        name = f"senzir.plugins.{shortname}"
        for i in reversed(range(len(senzir._event_builders))):
            ev, cb = senzir._event_builders[i]
            if cb.__module__ == name:
                del senzir._event_builders[i]
    except BaseException:
        raise ValueError
