        ...
        **kwargs,
        ...
        def decorator(func):  # sourcery no-metrics
            async def wrapper(check):
                ...
                try:
                    ...
                    if not disable_errors:
                        ...
                        text += f" عذرا قم بكتابه الأمر بشكل صحيح راجع : @senzir ** "
            ...
            if pattern is not None:
                ...
                if edited:
                    senzir.add_event_handler(func, events.MessageEdited(**kwargs))
                senzir.add_event_handler(func, events.NewMessage(**kwargs))
            return wrapper
    ...
    def bot_cmd(
        ...
        **kwargs,
        ...
        def decorator(func):
            async def wrapper(check):
                try:
                    ...
                    if not disable_errors:
                        ...
                        text += f" عذرا قم بكتابه الأمر بشكل صحيح راجع : @albrans0 ** "
                        await check.client.send_message(
                            Config.PRIVATE_GROUP_BOT_API_ID, text, link_preview=False
                        )
            ...
            if edited is True:
                senzir.tgbot.add_event_handler(func, events.MessageEdited(**kwargs))
            else:
                senzir.tgbot.add_event_handler(func, events.NewMessage(**kwargs))
