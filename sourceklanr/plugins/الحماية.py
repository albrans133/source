...
@senzir.iq_cmd(outgoing=True, func=lambda e: e.is_private, edited=False, forword=None)
...
@senzir.tgbot.on(CallbackQuery(data=re.compile(rb"show_pmpermit_options")))
...
@senzir.tgbot.on(CallbackQuery(data=re.compile(rb"to_enquire_something")))
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
        text = "**⁂ ⦙   عـذرا ، هـذه الـخـيـارات لـلـمـسـتـخـدم الـذي يـراسـلـك 🧸♥️**"
        return await event.answer(text, cache_time=0, alert=True)
...
@senzir.tgbot.on(CallbackQuery(data=re.compile(rb"to_request_something")))
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
        text = "**⁂ ⦙   عـذرا ، هـذه الـخـيـارات لـلـمـسـتـخـدم الـذي يـراسـلـك 🧸♥️**"
        return await event.answer(text, cache_time=0, alert=True)
...
@senzir.tgbot.on(CallbackQuery(data=re.compile(rb"to_chat_with_my_master")))
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
        text = "**⁂ ⦙   عـذرا ، هـذه الـخـيـارات لـلـمـسـتـخـدم الـذي يـراسـلـك 🧸♥️**"
        return await event.answer(text, cache_time=0, alert=True)
...
@senzir.tgbot.on(CallbackQuery(data=re.compile(rb"to_spam_my_master_inbox")))
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
        text = "**⁂ ⦙   عـذرا ، هـذه الـخـيـارات لـلـمـسـتـخـدم الـذي يـراسـلـك 🧸♥️**"
        return await event.answer(text, cache_time=0, alert=True)
    ...
    sqllist.rm_from_list("pmoptions", event.query.user_id)
    await event.edit(text)
@senzir.on(admin_cmd(pattern="الحماية (تشغيل|ايقاف)(?: |$)(.*)"))
...
async def pmpermit_on(event):
    ...
    if input_str == "ايقاف":
        ...
        await edit_delete(event, "**⁂ ⦙  امـر الـحـمـايـة بـالـفـعـل مُـمَـكـن لـحـسـابـك  🌿**")
@senzir.on(admin_cmd(pattern="(ق|قبول)(?:\s|$)([\s\S]*)"))
...
@senzir.on(admin_cmd(pattern="(ر|رفض)(?:\s|$)([\s\S]*)"))
async def disapprove_p_m(event):
    ...
    if reason == "الكل":
        ...
        return await edit_delete(event, "**⁂ ⦙   حــسـنـا تــم رفـض الـجـمـيـع بــنـجـاح 💯**") # CODE BY : T.me/IC_X_K
    if not reason:
        reason = "**⁂ ⦙  لـم يـذكـر 💭 **"
...
@senzir.on(admin_cmd(pattern="مرفوض(?:\s|$)([\s\S]*)"))
...
@senzir.on(admin_cmd(pattern="مقبول(?:\s|$)([\s\S]*)"))
...
@senzir.on(admin_cmd(pattern="المقبولين(?: |$)(.*)"))
