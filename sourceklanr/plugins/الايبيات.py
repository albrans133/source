...
@senzir .on(admin_cmd(pattern="جلب لقطات(?:\s|$)([\s\S]*)"))    
async def collage(event):
    ...
    for files in (catsticker, collagefile, endfile):
        if files and os.path.exists(files):
            os.remove(files)
@senzir .on(admin_cmd(pattern=r"رابط تطبيق ([\s\S]*)"))
...
@senzir .on(admin_cmd(pattern="الوقت(?:\s|$)([\s\S]*)(?<![0-9])(?: |$)([0-9]+)?"))
...
@senzir .on(admin_cmd(pattern="وقتي(?:\s|$)([\s\S]*)"))
async def _(event):
    ...
    await event.delete()
@senzir .on(admin_cmd(pattern=r"الاذان(?: |$)(.*)"))
async def get_adzan(adzan):
    ...
    senzir result = f"<b>اوقـات صـلاه المـسلمين 👳‍♂️ </b>\
...
@senzir .on(admin_cmd(pattern=r"كورونا(?:\s|$)([\s\S]*)"))
...
@senzir .on(admin_cmd(pattern=r"معلومات الاغنيه(?: |$)(.*)"))
...
@senzir .on(admin_cmd(pattern=r"كوكل بحث ([\s\S]*)"))
...
@senzir .on(admin_cmd(pattern=r"البحث العام(?: |$)(.*)"))
async def _(event):
    start = datetime.now()
...
@senzir .on(admin_cmd(pattern=r"البحث اونلاين(?:\s|$)([\s\S]*)"))
async def google_search(event):
    ...
    await event.delete()
@senzir .on(admin_cmd(pattern="تخزين الصوت(?: |$)(.*)"))
async def iq(event):
...
@senzir .on(admin_cmd(pattern="اضف الصوت(?: |$)(.*)"))
async def iq(event):
    ureply = await event.get_reply_message()
...
@senzir .on(admin_cmd(pattern="تحويل صوره(?: |$)(.*)"))
async def _(event):
    reply_to_id = await reply_id(event)
    reply = await event.get_reply_message()
    if not reply:
...
@senzir .on(admin_cmd(pattern="تحويل ملصق(?: |$)(.*)"))
async def _(event):
    reply_to_id = await reply_id(event)
    reply = await event.get_reply_message()
    if not reply:
    ...
    await output[0].delete()
@senzir .on(admin_cmd(pattern="تحويل (صوت|بصمه)(?: |$)(.*)"))
async def _(event):
    ...
    try:
        ...
        if os.path.exists(new_required_file_name):
            ...
            await event.delete()
@senzir .on(admin_cmd(pattern="تحويل متحركة ?([0-9.]+)?$"))
async def _(event):
    reply = await event.get_reply_message()
    ...
    for i in [inputfile, outputfile]:
        if os.path.exists(i):
            os.remove(i)
@senzir .on(admin_cmd(pattern="تحويل فديو دائري(?: |$)((-)?(s)?)$"))
async def pic_gifcmd(event):  # sourcery no-metrics
    ...
    for i in [final, "Output.gif", meme_file, PATH, final]:
        if os.path.exists(i):
            os.remove(i)
@senzir .on(admin_cmd(pattern="تحويل ملصق دائري ?((-)?s)?$"))
async def video_catfile(event):  # sourcery no-metrics
    ...
    await catevent.delete()
@senzir .on(admin_cmd(pattern="تحويل ملف ([\s\S]*)"))
async def get(event):
    name = event.text[5:]
    if name is None:
    ...
    if m.text:
        ...
        await edit_or_reply(event, "**✘ ⦙ قم بالـرد على الرسالـة لتحويلها الى ملف**")
@senzir .on(admin_cmd(pattern="بورن(?:\s|$)([\s\S]*)"))
...
@senzir .on(admin_cmd(pattern="(طقس|الطقس)(?:\s|$)([\s\S]*)"))    
...
@senzir .on(admin_cmd(pattern="تحويل رساله(?: |$)(.*)"))
async def get(event):
    reply = await event.get_reply_message()
    mediatype = media_type(reply)
    ...
    if os.path.exists(file_loc):
        os.remove(file_loc)
@senzir .on(admin_cmd(pattern="تحويل ملف صوره(?: |$)(.*)"))
async def on_file_to_photo(event):
    ...
    try:
        ...
        return
    await catt.delete()
@senzir .on(admin_cmd(pattern="تحويل ملصق متحرك(?:\s|$)([\s\S]*)"))
async def _(event):  # sourcery no-metrics
    ...
    for files in (catgif, catfile):
        if files and os.path.exists(files):
            os.remove(files)
@senzir .on(admin_cmd(pattern="تحويل متحركه(?: |$)((-)?(r|l|u|d|s|i)?)$"))
async def pic_gifcmd(event):  # sourcery no-metrics
    reply = await event.get_reply_message()
    mediatype = media_type(reply)
    ...
    for i in [final, "Output.gif", imag[1]]:
        if os.path.exists(i):
            os.remove(i)
@senzir .on(admin_cmd(pattern="مدينه الطقس(?:\s|$)([\s\S]*)"))    
...
@senzir .on(admin_cmd(pattern=r"(ت(لي)?ج(راف)?) ?(م|ك|ميديا|كتابه)(?:\s|$)([\s\S]*)"))
async def _(event):
...
@senzir .on(admin_cmd(pattern="تحويل فديو متحركه ?([0-9.]+)?$"))
async def _(event):
    reply = await event.get_reply_message()
    mediatype = media_type(event)
    if mediatype and mediatype != "video":
    ...
    for i in [inputfile, outputfile]:
        if os.path.exists(i):
            os.remove(i)
@senzir .on(admin_cmd(pattern="طقوس(?:\s|$)([\s\S]*)"))    
async def _(event):
    reply_to_id = await reply_id(event)
    input_str = event.pattern_match.group(1)
    if not input_str:
