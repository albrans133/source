...
@senzir .iq_cmd(pattern="تكرار حزمه الملصقات$",)
...
@senzir .iq_cmd(    pattern="تكرار_احرف ([\s\S]*)",)
...
@senzir .iq_cmd(    pattern="تكرار_كلمه ([\s\S]*)",)
async def tmeme(event):
...
@senzir .iq_cmd(pattern=f"{MUQT} ([\s\S]*)",)
...
@senzir .iq_cmd(pattern=f"{UNMUQT} ?(.*)",)
async def spammer(event):
    reply = await event.get_reply_message()
    await event.delete()
    delgvar("spamwork")
