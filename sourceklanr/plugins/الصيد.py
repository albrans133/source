...
@senzir .on(events.NewMessage(outgoing=True, pattern=r'.اغلاق الصيد'))
...
async def PhishingHundler(event):
    ...
    if PhishType not in valid_types:
        ...
        if workers.get(PhishType).get('worker') == False:
            ...
            try:
                channel = await event.client(CreateChannelRequest(title=f'صيد : {PhishType}', about=f'senzir '))
...
@senzir .on(events.NewMessage(outgoing=True, pattern=r'.سحب ?(.*)'))
...
# USERNAMES UNCHECK
@senzir .on(events.NewMessage(outgoing=True, pattern=r'.اغلاق سحب ?(.*)'))
