...
new_senzir = TelegramClient(StringSession(Config.STRING_SESSION), Config.APP_ID, Config.API_HASH)

async def PyStart():
    global senzir_py
    try:
        await new_senzir.start()
        senzir_py = PyTgCalls(new_senzir)
        await senzir_py.start()
...
async def JoinThenStreamVideo(chat_id, StreamFile):
    global senzir_py
    await PyStart()
    await senzir_py.join_group_call(
        int(chat_id),
        AudioVideoPiped(
            StreamFile,
            HighQualityAudio(),
...
async def LeaveStream(chat_id):
    global senzir_py
    await senzir_py.leave_group_call(
        chat_id,
    )


# DOWNLOAD THEN STREAM AUDIO
@senzir.on(events.NewMessage(outgoing=True, pattern=r'.تشغيل صوتي'))
...
@senzir.on(events.NewMessage(outgoing=True, pattern=r'.تشغيل فيديو'))
...
# LEAVE STREAM
@senzir.on(events.NewMessage(outgoing=True, pattern=r'.اغلاق البث'))
